...
@section('content')
    ...
    <main class="max-w-7xl mx-auto px-6 mt-10">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Posts em: {{ $tag->name }}</h1>
        ...
        <div class="grid md:grid-cols-3 gap-10">
            ...
            <section class="md:col-span-2 space-y-6">
                ...
                @forelse($posts as $post)
                    <article
                        class="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm hover:shadow-lg transition dark:bg-neutral-950 dark:border-neutral-800">
                        @if ($post->cover_path)
                        ...
                        <a href="{{ route("post.show", $post->slug) }}"
                            class="text-2xl font-semibold text-gray-900 hover:text-emerald-600 cursor-pointer dark:text-white dark:hover:text-emerald-600">{{ $post->title }}</a>
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            ...
                            <div class="text-sm text-gray-500 hover:text-emerald-600"><i class="fas fa-heart mr-1"></i>
                                <span class="font-semibold">{{ $post->likedByUsers->count() }}</span>
                            </div>
                        </div>
                    </article>
                @empty
                    <div
                        class="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm text-center dark:bg-neutral-950 dark:border-neutral-800 dark:text-white">
            ...
            <aside class="space-y-8 mb-8">
                <div
                    class="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm dark:bg-neutral-950 dark:border-neutral-800">
                    <h3 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white"><i
                            class="fas fa-user-friends mr-2"></i>Autores
                        Populares</h3>
                    ...
                    <ul class="space-y-4">
                        @foreach ($popularAuthors as $author)
                            <li class="flex items-center gap-4">
                                <img src="{{ !$author->cover_path ? 'https://ui-avatars.com/api/?name=' . urlencode($author->name) . '&background=009966&color=fff' : asset('storage/' . $author->cover_path)  }}"
                                    class="w-10 h-10 rounded-full object-cover" alt="avatar">
                                <div>
                ...
                <div
                    ...
                    <ul class="space-y-4">
                        @foreach ($popularPosts as $popularPost)
                            <li
                                class="bg-gray-100 border border-gray-200 p-4 rounded-lg dark:bg-neutral-800 dark:border-neutral-700">
                    ...
                    <ul class="space-y-2">
                        @forelse($categories as $category)
                            <li><a href="{{ route("category.show", $category->slug) }}"
                                    class="text-emerald-600 hover:underline">{{ $category->name }}</a></li>
                        @empty
                ...
                <div
                    class="bg-white border border-gray-200 p-6 rounded-2xl shadow-sm dark:bg-neutral-950 dark:border-neutral-800">
                    <h3 class="text-lg font-semibold mb-3 text-gray-800 dark:text-white"><i class="fas fa-hashtag mr-2"></i>Tags
                    </h3>
