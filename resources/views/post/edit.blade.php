                ...
                <form action="{{ route('post.update', $post->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    @if ($post->cover_path)
                        <img src="{{ asset('storage/' . $post->cover_path) }}" alt="{{ $post->title }}"
                            class="mt-2 w-full h-auto rounded-md object-cover">
                        <button type="button" class="mt-2 mb-4 text-red-600 hover:text-red-800 focus:outline-none"
                            onclick="event.preventDefault(); document.getElementById('delete-cover-form').submit();">
                            Remover Imagem
                        </button>
                    @endif
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <button type="submit"
                            class="bg-emerald-600 text-white px-4 py-2 rounded-lg hover:bg-emerald-700 transition">Editar</button>
                    </div>
                </form>
                <form id="delete-cover-form" action="{{ route('post.destroyImage', $post->id) }}" method="POST" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
